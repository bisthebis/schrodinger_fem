...
class AbstractMesh:
    """
    Abstract interface for a mesh consisting of a list of
    elements (self.elements), all having DOFs mapping with
    DOFS < self.do_counter.
    ...
    "Element" must :
        ...
        - Have an attribute "ddls" containing the list of global DOFs to
            influence
    """
        ...
        for elem in self.elements:
            ...
            for i in range(0, elem_dof_counter):
                for j in range(0, elem_dof_counter):
                    self.H[elem.ddls[i], elem.ddls[j]] = \
                        self.H[elem.ddls[i], elem.ddls[j]] + H[i, j]
                    self.M[elem.ddls[i], elem.ddls[j]] = \
                        self.M[elem.ddls[i], elem.ddls[j]] + M[i, j]
    ...
    def remove_DOF(self, i):
        """
        Removes the row and column related to a given DOF.
        Be careful : this changes the ordering of DOFs !
